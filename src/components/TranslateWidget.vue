...
function initGoogleTranslate() {
  ...
  ;(window as any).googleTranslateElementInit = () => {
    new (window as any).google.translate.TranslateElement(
      {
        pageLanguage: 'en',
        includedLanguages: 'en,hi,gu,mr,pa,es,fr,de,ja,zh-CN',
...
<template>
  <div class="relative inline-block text-left print:hidden">
    ...
    <button
      type="button"
      class="px-2 py-1 text-xs font-semibold rounded-md bg-gray-500/10 hover:bg-gray-500/20 text-current transition-colors border-none! cursor-pointer inline-flex items-center gap-1 opacity-75 hover:opacity-100"
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        <div
          v-if="isOpen"
          class="fixed inset-0 z-50 flex items-start justify-end p-4 pt-16 bg-black/60 backdrop-blur-xs"
          @click.self="isOpen = false"
        >
          <div class="relative w-72 bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-800 p-4 modal-popup-box">
            <div class="flex items-center justify-between border-b border-gray-200 dark:border-gray-800 pb-2.5 mb-3">
              ...
              <button
                type="button"
                class="text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 text-sm font-bold border-none! bg-transparent cursor-pointer"
                @click="isOpen = false"
              >
                ✕
              </button>
            ...
            <div class="space-y-1.5 max-h-72 overflow-y-auto pr-1">
              <button
                ...
                :key="lang.code"
                type="button"
                class="w-full px-3 py-2 rounded-lg text-left text-xs font-medium bg-gray-500/5 hover:bg-gray-500/15 transition-colors border-none! cursor-pointer flex items-center justify-between text-gray-800 dark:text-gray-200"
